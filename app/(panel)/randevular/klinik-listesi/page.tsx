...
// --------- Types (UPDATE: Gelişmiş, ISO tarih) ----------
type Appointment = {
  id: number;
  hasta_id: number;
...
// -- DİNAMİK İŞLEM ve AMELİYAT LİSTELERİ (initial, useState ile doldurulacak) --
...
// ---- Helper: Tarih Saat Formatları (ISO) ----
...
// --- Searchable Combobox Patient Picker ---
// ... same as original ...
...
// ---- Multi-Select Combobox for İşlem (Dinamik ve Yeni Ekle Özellikli) ----
// ... same as original ...
...
// ---- Yeni Randevu Modal (Super Modal!) ----
function AppointmentCreateModal({
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title="Yeni Randevu Oluştur" wide>
        <form className="p-6 flex flex-col gap-4" onSubmit={onSave} autoComplete="off">
          ...
          {/* İşlem Seçimi - Görsel olarak ameliyat listesiyle eş: Yan yana radio card, ikincisi pasif */}
          <div>
            <input type="hidden" name="tur" value="Klinik" />
            <label className="block text-xs text-slate-500 mb-2 font-medium">
              İşlem Türü
            </label>
            <div className="flex gap-4 mb-1">
              {/* Klinik İşlem Radio Card (aktif/seçili/mavi) */}
              <button
                type="button"
                className={clsx(
                  ...
                  "border-blue-600 bg-blue-50 text-blue-700", // always selected
                  "cursor-default",
                )}
                aria-pressed={true}
                tabIndex={-1}
              ...
              >
                <Syringe size={25} className="mr-2 transition text-blue-500" />
                Klinik İşlem
              </button>
              {/* Ameliyat Radio Card (pasif/gri/disabled) */}
              <button
                type="button"
                className={clsx(
                  "flex-1 flex items-center gap-2 px-0 py-3 rounded-xl border-2 text-lg font-semibold transition group justify-center shadow-sm",
              ...
              >
                <Activity size={24} className="mr-2 transition text-slate-400" />
                Ameliyat
              </button>
            </div>
            {/* Multi-Select Combobox (her zaman sabit Klinik işlemler) */}
            <MultiSelectIslemCombobox
              options={klinikIslemler}
              setOptions={setKlinikIslemler}
              selected={form.secilen_islemler}
              setSelected={(arr) =>
                setForm((prev: any) => ({ ...prev, secilen_islemler: arr }))
              }
              disabled={saving}
...
// ... unchanged ...
function TimelineList({ appointments, onDelete, deletingId }: { appointments: Appointment[]; onDelete: (id: number) => void; deletingId: number | null; }) {
  return (
...
// ---------- Detay/Düzenleme Modal (değişmedi) ----------
// ... unchanged ...
...
// ----- Main Page -----
export default function KlinikListesiPage() {
  // ------ STATES ------
  ...
  const [form, setForm] = useState<{
    ...
    tarih: "",
    saat: "",
    tur: "Klinik", // Varsayılan olarak 'Klinik'
    secilen_islemler: [],
    notlar: "",
    islem: "",
  });
  ...
  // Dinamik Klinik İşlem listeleri (ameliyat burada olmasa da state tutalım)
  const [klinikIslemler, setKlinikIslemler] = useState<string[]>(() => INITIAL_KLINIK_ISLEMLER.slice());
  ...
  async function fetchRandevular() {
    ...
    const { data, error } = await supabase
      ...
      .eq("tur", "Klinik") // Sadece Klinik işlemleri getir
  ...
  // --- Randevu Ekle (INSERT: gereksinim formatı) ---
  async function handleSave(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    ...
    // Klinik Listesi sayfasında tur her zaman Klinik!
    const turValue = "Klinik";
  ...
  return (
    <div className="max-w-5xl mx-auto px-1 md:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3 sm:gap-0">
        <div>
          ...
          <div className="text-slate-500 text-sm mt-1">
            Planlanmış klinik işlemleri buradan yönetin.
          </div>
        </div>
      ...
      <AppointmentCreateModal
        ...
        klinikIslemler={klinikIslemler}
        setKlinikIslemler={setKlinikIslemler}
